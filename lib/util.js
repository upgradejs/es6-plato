...
async function getLockFilesExistence(projectRootPath) {
  let packageLockExist = false;
  let yarnLockExist = false;

  yarnLockExist = !!((await statAsync(path.resolve(projectRootPath, 'yarn.lock')).catch(() => false)));
  packageLockExist = !!((await statAsync(path.resolve(projectRootPath, 'package-lock.json')).catch(() => false)));
