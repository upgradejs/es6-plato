...
var util = require('./util'),
  ...
  overallReporters = {
    audit: require('./reporters/dependencies/audit'),
    compatibility: require('./reporters/dependencies/compatibility'),
    outdated: require('./reporters/dependencies/outdated')
  };

var overviewTemplate = __dirname + '/templates/overview.html',
	displayTemplate = __dirname + '/templates/display.html',
  dependenciesAuditTemplate = __dirname + '/templates/dependencies-audit.html',
  dependenciesCompatibilityTemplate = __dirname + '/templates/dependencies-compatibility.html',
  dependenciesOutdatedTemplate = __dirname + '/templates/dependencies-outdated.html',
...
async function inspect(files, outputDir, options, done) {
  ...
  let overallReportersResults = {};

  if (options.projectRoot) {
    for (const [name, { process }] of Object.entries(overallReporters)) {
      overallReportersResults[name] = await process(options);
    }
  }
  ...
  if (!fileOutputDir) {
		...
		fs.mkdirp(fileOutputDir, async function() {
          ...
          const dynamicReportsPairs = [
            [dependenciesAudit, dependenciesAuditTemplate],
            [dependenciesCompatibility, dependenciesCompatibilityTemplate],
            [dependenciesOutdated, dependenciesOutdatedTemplate]
          ];
          ...
          if (options.projectRoot) {
            dynamicReportsPairs.forEach(([rpt, template]) => {
              writeOverview(
                rpt,
                overviewReport,
                {
